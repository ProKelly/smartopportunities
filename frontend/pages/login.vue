  ...
  <div class="relative min-h-screen bg-navy-950 flex items-center justify-center px-4 overflow-hidden">
    ...
    <div class="relative z-10 w-full max-w-md">
      ...
      <div class="rounded-2xl bg-navy-900/40 border border-navy-800/50 p-6 sm:p-8 backdrop-blur-sm transition-all hover:border-navy-700/70">
        ...
        <h1 class="text-2xl font-display font-medium text-parchment tracking-tight">Welcome back</h1>
        <p class="mt-1.5 text-sm text-navy-400">Sign in to continue your journey</p>
        ...
        <form class="mt-6 space-y-4" @submit.prevent="onSubmit">
          ...
          <div class="flex items-center justify-between">
            <label class="flex items-center gap-2 text-xs text-navy-400 cursor-pointer">
              <input type="checkbox" class="rounded border-navy-700 bg-navy-950 text-chart-400 focus:ring-chart-400/20" />
              <span>Remember me</span>
            </label>
            <NuxtLink to="/forgot-password" class="text-xs text-chart-400 hover:text-chart-300 transition-colors">
              Forgot password?
            </NuxtLink>
          </div>
          ...
          <button 
            ...
            :disabled="loading"
          >
            <ArrowRightOnRectangleIcon v-if="!loading" class="h-4 w-4" />
        ...
        <p class="mt-6 text-center text-sm text-navy-400">
          New here?
          <NuxtLink to="/signup" class="text-chart-400 hover:text-chart-300 transition-colors font-medium">
            Create an account
          </NuxtLink>
        </p>
      </div>
